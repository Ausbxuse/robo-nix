...
Graphics support is selected through named runtime pieces such as `desktop-gl`, `host-nvidia-gl`, and `qt6`. You may see these names in `robo.nix` or `robo check` output.
...
`desktop-gl` is the default desktop graphics component. It provides Nix-managed userspace graphics libraries for OpenGL/EGL clients that may run through X11, XWayland, or native Wayland depending on the app and host session.

`desktop-gl` points EGL at a matching Nix Mesa GLVND vendor file by default.
