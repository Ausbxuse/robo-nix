...
Reviewer concerns raised after this change should be recorded here first.
They should not be implemented one by one during review. The next change
...
- Maintain an effective root `AGENTS.md` as part of the rebuild. It should be
  refined during changelog-backed changes when durable working rules emerge, not left as a
  stale copy of the old repository guidance.

Proposed handling:

- Add a root `AGENTS.md` in the next implementation change before growing
  product code further.
- Keep `AGENTS.md` short and operational: project north star, code ownership
  boundaries, changelog/review rules, verification expectations, and known
  failure-mode policy.
- Keep change-specific debate in `docs/changelog/YYYY-MM-DD-short-title.md`; promote
  only durable lessons into `AGENTS.md`.
- Review `AGENTS.md` as part of each change close-out, and update it only
  when the change would prevent future churn or rediscovery.
- Reconsider embedded generated Nix strings in Rust. The initial CLI
  writes `flake.nix` and `robo.nix` from Rust string literals, which is
  tolerable for the initial scope but likely not a clean long-term boundary.
...
## Candidate Scope For Shell-Centered Bootstrap
...
- Keep the root flake small: one dev shell, one formatter/check path, and any
  narrow checks needed for the change. Avoid reintroducing the old repository
  support framework.
...
- Add minimum docs by audience: a user getting-started page for the four-command
  workflow, a developer overview explaining the changelog process and code
  boundaries, and a short root `AGENTS.md`.
- Remove the initial `robo check` surface for now. Reintroduce it only if
...
- What is the cleanest rule data format for the shell-centered bootstrap change while keeping future
...
- `include_str!` clarification: this means checked-in template files are compiled
  ...
  the shell-centered bootstrap change.
...
- Changelog-backed changes should ship fast, break fast, and iterate fast while preserving
  minimal completeness.

Consistency checks before implementation:

- The initial scope currently includes `robo check`; review says no `robo check` yet.
  The shell-centered bootstrap change should remove it from the public surface.
- The initial scope currently hardcodes Python `3.11`; review says there should be
  no Rust default. The shell-centered bootstrap change should read `.python-version` and fail clearly
  if absent.
...
- `robo init` should be removed entirely. The shell-centered bootstrap change should reshape the CLI
...
- Do not eagerly create or refresh `flake.lock` before it is needed. Running
  ...
  concrete need.
- Initial component names for the shell-centered bootstrap change are `python-uv`, `native-build`, and
...
Before the shell-centered bootstrap change, confirm whether this is the right minimal product core:
...
- Should root `AGENTS.md` maintenance be a required close-out check for every
  changelog-backed change?
- Should the shell-centered bootstrap change move generated Nix into template files before any new
  runtime behavior is added?
- Are the candidate shell-centered bootstrap boundaries above correct, especially keeping
