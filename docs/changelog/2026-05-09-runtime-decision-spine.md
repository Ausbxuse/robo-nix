# 2026-05-09 - Runtime Decision Spine
...
- Existing dirty worktree changes are preserved. This change builds on the
  environment isolation, inference, and host CUDA bridge work from the
  2026-05-09 environment isolation entry.
...
- Pixi's activation and workspace code keeps shell state explicit and keyed by
  real inputs. This change mirrors that by fingerprinting runtime files plus
...
- devenv's Nix modules keep component-owned behavior close to the Nix layer and
  emit unsets for removed shell variables. This change keeps component env
...
- [x] Record source-survey decisions in this changelog entry.
