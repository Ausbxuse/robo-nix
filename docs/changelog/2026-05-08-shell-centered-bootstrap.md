# 2026-05-08 - Shell-Centered Bootstrap

## Goal

Replace the initial `init`/`check` surface with a shell-centered workflow:
...
`robo` should not eagerly refresh it in this change.
...
Pending concerns from this change should be appended here during review and
handled in the next changelog-backed change as a coherent change.
