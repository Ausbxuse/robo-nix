...
cargo run -q -p robo-cli -- --no-color diagnose "$glibc_log" >"$tmpdir/glibc.out"
...
printf '%s\n' "GLFWError: (65542) b'EGL: Failed to get EGL display: Success'" |
	cargo run -q -p robo-cli -- --no-color diagnose - >"$tmpdir/egl.out"
...
printf '%s\n' "Failed to get EGL display" |
	cargo run -q -p robo-cli -- --no-color diagnose - >"$tmpdir/egl-short.out"
...
printf '%s\n' "Failed EGL display" |
	cargo run -q -p robo-cli -- --no-color diagnose - >"$tmpdir/egl-shorter.out"
...
printf '%s\n' "EGL display" |
	cargo run -q -p robo-cli -- --no-color diagnose - >"$tmpdir/egl-search.out"
...
printf '%s\n' "Could not find Qt6Config.cmake" |
	cargo run -q -p robo-cli -- --no-color diagnose --json - >"$tmpdir/qt.json"
...
printf '%s\n' "EGL display" |
	cargo run -q -p robo-cli -- --no-color diagnose --json - >"$tmpdir/egl-search.json"
...
printf '%s\n' "plain pytest assertion failure" |
	cargo run -q -p robo-cli -- --no-color diagnose - >"$tmpdir/unknown.out"
