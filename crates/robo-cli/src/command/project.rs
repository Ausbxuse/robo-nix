...
pub(crate) fn run_project_up(
    ...
    if let Some(path) = auto_host_cuda_driver_path(&env) {
        status(
            config,
            &format!("up: detected NVIDIA CUDA driver at {path}"),
        );
    }
    if let Some(path) = auto_host_graphics_path(&env) {
        status(
            config,
            &format!("up: detected NVIDIA graphics manifests at {path}"),
        );
    }
...
fn materialize_shell_env(
    ...
    append_host_cuda_driver_bridge(&mut env);
    append_host_graphics_bridge(&mut env);
...
fn append_host_cuda_driver_bridge(envs: &mut Vec<(String, String)>) {
...
fn runtime_needs_host_cuda_driver(runtime: &crate::runtime::ProjectRuntime) -> bool {
    runtime.cuda_wheel_version.is_some()
        || runtime.components.iter().any(|component| {
            matches!(component.as_str(), "cuda-toolkit" | "isaac-sim")
        })
}

fn append_host_graphics_bridge(envs: &mut Vec<(String, String)>) {
    let runtime = crate::runtime::read_project_runtime();
...
fn auto_host_cuda_driver_path(envs: &[(String, String)]) -> Option<&str> {
    shell_env_value(envs, "ROBO_NIX_HOST_LIBCUDA_AUTO").map(String::as_str)
}

fn auto_host_graphics_path(envs: &[(String, String)]) -> Option<&str> {
